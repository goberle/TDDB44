                        ...
                        ast_expr_list *actuals)
{
    ...
    if (formals == NULL)
    {
        type_error(env->pos) << "Too many parameters\n";
        return false;
    }

    if (actuals == NULL)
    {
        type_error(env->pos) << "Missing parameter\n";
        return false;
    }
    ...
    if (actual_type != formals->type)
    {
        ...
        {
            type_error(env->pos) << "Expected parameter of type integer, got real\n";
            return false;
        }
    }
...
sym_index ast_assign::type_check()
{
    ...
    if (lhs_type == integer_type && rhs_type != integer_type)
        type_error(this->pos) << "Assignment to integer variable must be of integer type\n";
